...
\begin{itemize}
  \item Tukey-type comparison within a single well across different plates
  \item Dunnett-type comparison between wells from the same plate
  \item Pairs-type comparison of groups defined by specific combinations of metadata entries
  \item User-defined comparisons of interest
\end{itemize}
% redundant, please delete or link (make it independent of ordering)


\subsubsection[Tukey-type comparison within a single well]{Tukey-type comparison within a single well across different plates}\label{Tukey-type comparison}
% Heading should be more general: Tukey-type comparison, with this just as an example.
% Do not indicate only this kind of comparison was possible!
...
% Heading should be more general: Dunnett-type comparison, with this just as an example.
% Do not indicate only this kind of comparison was possible!
...
\subsubsection[Pairs-type comparison of specific wells]{Pairs-type comparison of groups defined by specific combinations of metadata entries}\label{specific-well-Pairs}
% Heading should be more general: Pairs-type comparison, with this just as an example.
% Do not indicate only this kind of comparison was possible!
...
%\tiny
%<<label=opm-mcp-show-data-display, echo=FALSE>>=
%opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ Species, m.type = "aov",
%        linfct = c(Tukey = 1), output = "data", full = FALSE)
%@
%\normalsize
...
<<label=opm-mcp-linfct-model>>=
opm_mcp(vaas.G06, model = ~ J(Well, Species), m.type = "aov",
...
% TODO: makes not much sense, output = "linfct" is only for showing the generated intermediary object
