...
\rcsmark{
Update needed. 
Original graphics file should be made available to all contributors.
}
...
\rcscom{
Lea \& Johannes, please check whether the phrasing of this subsection is OK now.
...
\rcsmark{Lea, the font size is way too small. Revise here or suggest changes of function.}
}
\end{figure}
...
To demonstrate the functionality of \code{opm\_mcp()}, let  us make use of the example addressed above. 
Imagine the comparison of four different bacteria using GEN-III microplates.
For each bacterial strain, ten replicates have been performed.
Do these four bacteria differ in, e.g., the mean value of, e.g., curve parameter A of, e.g, well A1?
...
Let us first subset from the data set \code{vaas\_et\_al} the ten GEN-III plates from the first technical replicates and let us also subset for the curves from well G06.
<<results=hide, width=8>>=
xyz <- subset(vaas_et_al[, , "G06"], list(Experiment = "First replicate"))
@

Let us plot these data (see Figure~\ref{fig:xy_plot_mcp}).
For each of the four strains, ten curves are shown. 
...
The groups to be compared are defined by the argument ``model''.

\rcscom{Die anderen Argumente sollten hier auch erklaert werden, oder?}

<<results=hide, width=8>>=
(x <- opm_mcp(xyz, model = ~ Strain, m.type = "aov",
