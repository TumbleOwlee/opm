...
<<label=plot-vaas-subset-dummy, eval=FALSE, echo=FALSE>>=
...
The code below summarizes the respective data frame, which contains the A values (maximum height) of the well G06 ($\alpha$-Keto-Glutaric Acid) from four strains and 10 plates, respectively:
...
In our example, a Tukey-type contrast matrix is used (a more detailed explanation of predefined contrast matrices is given in the help page of \code{multcomp::contrMat()}).
The test results in a set of six two-sided pairwise comparisons between all four strain means (all possible pairs).
The results of the comparison can be written into an object termed, e.g., \code{vaas.G06.mcp}:
...
%% TODO: LEA
%% please explain the meaning of linfct = 1 better!

The here used kind of \code{linfct} argument, a numeric vector, refers to the positions of the variables within \code{model}. Thus, by using $1$ the first and only term, ``Strain'', is selected.
The names of such a \code{linfct} object indicate the type of contrast used, here ``Tukey''.
\code{opm_opt("contrast.type")} would be inserted, if names were missing.
...
Similarly, the correct usage of the argument \code{linfct} can be checked as follows:
...
% TODO:LEA
% please state a short paragraph about the relationship between p-value and effect-size
...
The reference well can be either the negative or a positive control but also one substrate, for example, serving as a standard in a specific chemical group.
...
The value for the \code{linfct} argument can be constructed by typing \code{Dunnett}, and, separated by any sign, e.g. underscore (``\_''), the level name which should serve as the reference group in the contrast set. 

A summary of the resulting data frame, with well A03 chosen as the reference group, shows the ``A'' (maximum height) values from ten wells of all ten plates:
...
% this explanation is not correct or rather confusing

This can easily be performed by applying the specially designed \code{linfct} argument ``Pairs''. 
First, the user needs to take care that ``Well'' is part of the model and is joined with at least one other factor extracted from the metadata, in this case with ``Species''. 
This can, as usual, be achieved on-the-fly with the \code{J()} pseudofunction as shown below.

% TODO Johannes: please avoid redundancy in next two code chunks.
% Please avoid numbered code-chunk names, provide sensible labels.
...
Second, the setting \code{linfct = c(Pairs = 1)} ensures that for all levels present in the first (joined) component of the model, i.e, \code{Well}-wise, all pairwise comparisons are performed among the different groups present (here: the two species \emph{P. aeruginosa} and \emph{E. coli}).
After joining, the model contains only a single component, which is targeted using a numeric value of 1.
The name of the \code{linfct} value indicates the type of contrast. Unless explicitly specified, ``Pairs'' selects the first subcomponent of the selected (joined) model component for the comparisons.
The result of this analysis is shown below.

<<label=opm-mcp-linfct-Pairs-1c>>=
y <- opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ J(Well, Species), 
  m.type = "aov", linfct = c(Pairs = 1), full = FALSE)
@

% TODO Johannes: please avoid changing font sizes: it doesn't really look very nice.
% Turn too broad output off using full = FALSE.
...
For instance in the example shown above, by using \code{linfct = c(Pairs.Species = 1)} the metadata name \code{Species} can directly be addressed. 
Thus the result of 
...
% Please avoid changing font sizes, see above.
% Please show the code that yields some result (if the result is shown...).
...
% TODO Johannes: please avoid numbered code-chunk labels.
...
% TODO Johannes or Lea: I think the following examples are rather unlucky
% because joining metadata in that manner is not the method of choice.
