...
The correct usage of the argument \code{model} can be checked by outputting just the converted argument:
...
In our example, the results from the statistical calculations indicate that all significant differences are also \textit{highly} significant (p < 0.001).
...
Please note a speciality of the code above.
...
A summary of the resulting data frame, with well A3 chosen to be the reference group, shows the ``A'' values from ten wells of all ten plates:
...
The object resulting from the test is of class \code{glht} and can be plotted as usual using \code{plot()}.
