...
This has successfully been applied to slow-growing organisms in the genus \textit{Geodermatophilus}, which had to be measured three times consecutively in the OmniLog\textregistered \ instrument \citep{montero, montero2013}.
...
An experimental questions that could be addressed with these data might be ``Do these four bacteria differ in the mean value of curve parameter A on well G06''?
...
As shown in Figure~\ref{fig:ci_plot_mcp}, the results of the performed statistical test can be plotted using the methods available for objects of that class (see the help pages of \code{glht} for details).
\rcsmark{Too unspecific, provide help page(s) exactly.}
...
\rcscom{
Currently showing this output yielded unprintable characters!
Unless this can be fixed we cannot show the output of summary() here.
At the moment we could thus also omit \code{mcp.summary\$model\$call <- NULL}
}
...
The underlying contrast matrix can be viewed by entering
...
The function \code{contrMat()} from \pkg{multcomp} provides an overview of the predefined contrast types that can be used in the \code{opm\_mcp()} argument \code{mcp.def}.
...
\rcscom{
At this position is some hidden R code chunk in the LaTeX source code.
Shouldn't this be shown?
}
...
As mentioned above, the outcome can be visualized using the \code{plot()} method for \code{glht} objects:
...
The picture was obtained by running \code{opm\_mcp()} and then the the plotting function for the resulting object.
