...
The raw kinetic data can be plotted either as level plots or as XY plots.
The estimated curve parameters can be plotted either as confidence-interval plots, radial plots  or heat maps. 
See Section~\ref{methods plotting} for the underlying principles and Section~\ref{application plotting} for plotting example code. 

To statistically compare curve parameters, tools for the multiple comparison of groups means have been adapted to PM data.
This allows for testing statistical hypothesis involving groups of numerous plates as defined, e.g., \textit{via} the stored metadata entries.
