...
When performing PM experiments, one kind of standard analysis is the comparison of all wells against a single, selected well.
...
To determine the groups to be compared, \code{Well} has to be part of the model and joined (by function \code{J()}) with at least one other factor extracted from the metadata.
...
is the pairwise comparisons of the strains regarding the two species \textit{Escherichia coli} and \textit{Pseudomonas aeruginosa}.
