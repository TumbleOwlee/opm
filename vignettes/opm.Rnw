...
batch_opm_to_yaml(files, include = "*Example_?.csv.xz",
  ...
  outdir = ".", demo = TRUE)
@
The arguments \code{aggr.args} and \code{md.args} control aggregation and metadata incorporation, respectively.
Details on both processes are given below, and for the exact use of these arguments see the \pkg{opm} manual, entering \code{?batch\_opm\_to\_yaml}.
