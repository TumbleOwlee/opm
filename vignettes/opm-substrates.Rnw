...
We show how these data can be used to visualise results from PM experiments, including the outcome from advanced multiple-comparison statistics, within biochemical pathway maps.
...
Beside methods for assessing this information directly, this tutorial introduces strategies for visualising the measured PM results by mapping them on pathway maps.
Furthermore, analysis methods are described for modelling the effect of substrate features on the PM results and thus, e.g., for the identification of those pathways that are particularly suitable for visualising the PM results in pathway graphs.
...
Using the ``download'' argument, if KEGG drug or compound IDs have been selected, these can be downloaded from the KEGG server if the \pkg{KEGGREST} is available and converted into customised objects.
...
In conjunction with other \proglang{R} packages, it is possible to visualise PM results directly in pre-existing pathway maps as, for example, those from the KEGG database.
...
Visualisation of the results of an \proglang{opm\_mcp} analysis is also possible, which offers more (statistically interesting) opportunities for making sense of the PM data in the context of pathways.
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                ...
                \caption{Respiratory data (mean maximum height, discretised) of \textit{Escherichia coli} strain K12 mapped on the KEGG Galactose pathway.}
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                ...
                \caption{Respiratory data (mean maximum height, discretised) of \textit{Escherichia coli} type strain DSM 30083\textsuperscript{T} mapped on the KEGG Galactose pathway.}
...
Thus the full power of the functions from the \pkg{multcomp} package underlying \code{opm\_mcp} (see the main tutorial and the manual for details) is available when visualising PM data in pathway graphs.
...
\subsubsection{Visualisation of pathway maps in ``Graphviz'' layout}\label{pathview-denovo-layout}

In addition to the native KEGG visualisation, \code{pathview} can use the ``Graphviz''library for an alternative visualisation approach.
...
Respiratory data (mean maximum height) of \textit{Escherichia coli} strain K12 mapped on the KEGG Galactose pathway and rendered by \pkg{pathview} using the ``Graphviz'' layout engine.
Compare Figure~\ref{fig:1a-pathview-native-sep-strains}, which shows the native KEGG rendering of these data.
...
There are, unfortunately, only limited options for tuning the font sizes or the size of the symbols in the graphs.
Regarding the colour arguments, \rcsmark{TODO}.
The scaling \rcsmark{TODO}.
...
In the next coding example, we prepare a data matrix of differences between means and potential explanatory variables from the KEGG database.
...
The matrix has to be reduced to the non-\code{NA} rows, i.e. substrates that are not included in KEGG have to be removed.
